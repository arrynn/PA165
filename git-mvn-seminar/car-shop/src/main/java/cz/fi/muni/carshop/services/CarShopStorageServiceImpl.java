...
public class CarShopStorageServiceImpl implements CarShopStorageService {
    ...
    public void sellCar(Car car) throws RequestedCarNotFoundException {
        try {
            ...
            for (Car iteratedCar : carsOfSameType) {
                if (car.getPrice() == iteratedCar.getPrice() && car.getColor() == iteratedCar.getColor() && car.getConstructionYear() == iteratedCar.getConstructionYear()) {
                    carsOfSameType.remove(iteratedCar);
                }
            }
